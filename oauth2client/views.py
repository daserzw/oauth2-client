import random, hashlib, urllib.parse, requests, json, pprint
...
def index():
    ...
    full_as_auth_url_print = f"{as_auth_url}\n?response_type=code\n&client_id={client_id}\n&redirect_uri={urllib.parse.quote_plus(redirect_uri)}\n&state={state}\n&scope={scope}"

    full_as_auth_url = f"{as_auth_url}?response_type=code&client_id={client_id}&redirect_uri={urllib.parse.quote_plus(redirect_uri)}&state={state}&scope={scope}"
